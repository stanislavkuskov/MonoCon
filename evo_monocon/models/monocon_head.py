...
class AttnBatchNorm2d(nn.BatchNorm2d):
    ...
    def __init__(
        ...
        super(AttnBatchNorm2d, self).__init__(
            num_features,
            affine=False,
            eps=eps,
            momentum=momentum,
...
class MonoConHead(nn.Module):
    def __init__(self,
        ...
        self.loss_center2kpt_offset = L1Loss(loss_weight=1.)
        self.loss_kpt_heatmap = CenterNetGaussianFocalLoss(loss_weight=1.0)
        self.loss_kpt_heatmap_offset = L1Loss(loss_weight=1.)
        self.loss_dim = DimAwareL1Loss(loss_weight=1.)
        if 'Aware' in loss_dim['type']:
            self.dim_aware_in_loss = True
        else:
            self.dim_aware_in_loss = False
        self.loss_depth = LaplacianAleatoricUncertaintyLoss(loss_weight=1.0)
